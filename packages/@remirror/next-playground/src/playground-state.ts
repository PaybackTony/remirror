...
import { detectImportsNeedingTypes } from './vendor/type-acquisition';
...
class PlaygroundSingleton {
  ...
  setupEditor() {
    ...
    this.typescript.setDiagnosticsOptions({
  ...
  registerPrettierFormatter() {
    return this.languages.registerDocumentFormattingEditProvider('typescript', {
      provideDocumentFormattingEdits: async (model) => {
        // Load in prettier and the parser to be used.
        const prettier = await import('prettier/standalone');
        const babel = await import('prettier/parser-babel');

        // Get the response from prettier using the current text value from the
        // editor.
  ...
  createTypeGetter(model: Monaco.editor.ITextModel) {
    ...
    const getTypes = () => {
      detectImportsNeedingTypes(model.getValue(), addLibraryToRuntime, window.fetch.bind(window));
    };
