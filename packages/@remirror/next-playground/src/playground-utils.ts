 ...
 * info
 */
export function mapModuleNameToModule(name: string): string {
  const builtinModules: string[] = loadJson('./json/builtin-modules.json');

  return builtinModules.includes(name) ? 'node' : name;
}
 ...
 */
export function isDisallowed(name: string): boolean {
